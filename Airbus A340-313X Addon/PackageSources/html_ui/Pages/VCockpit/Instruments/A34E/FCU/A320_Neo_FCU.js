...
class A320_Neo_FCU_Component {
    ...
    set textValueContent(_textContent) {
        if (this.textValue != null) {
            diffAndSetText(this.textValue, _textContent);
        }
    }
    ...
    setTextElementActive(_text, _active) {
        if (_text != null) {
            diffAndSetAttribute(_text, "class", "Common " + (_active ? "Active" : "Inactive"));
        }
    }
    setElementVisibility(_element, _show) {
        if (_element != null) {
            diffAndSetStyle(_element, StyleProperty.display, _show ? "block" : "none");
        }
    }
    reboot() {
        this.init();
    ...
    refresh(_isActive, _isManaged, _showSelectedSpeed, _machActive, _value, _force = false) {
        if ((_isActive != this.isActive) || (_isManaged != this.isManaged) || (_showSelectedSpeed != this.showSelectedSpeed) || (_value != this.currentValue) || _force) {
            ...
            if (!this.isManaged) {
                ...
                this.textValueContent = (value + '').padStart(3, "0");
...
class A320_Neo_FCU_Heading extends A320_Neo_FCU_Component {
    ...
    onFlightStart() {
        ...
        if (!showSelectedHeading) {
            ...
            Coherent.call("HEADING_BUG_SET", 0, Math.round(simHeading));
        }
    }
    update(_deltaTime) {
        ...
        if (isLateralModeActive) {
            ...
            {
                if (isTRKMode) {
                    this.refresh(true, isManaged, true, showSelectedHeading, Simplane.getAutoPilotDisplayedHeadingValue(false));
                }
                else {
                    this.refresh(true, isManaged, false, showSelectedHeading, Simplane.getAutoPilotDisplayedHeadingValue(false));
                }
            }
        }
    }
...
class A320_Neo_FCU_Altitude extends A320_Neo_FCU_Component {
    ...
    refresh(_isActive, _isManaged, _value, _force = false) {
        if ((_isActive != this.isActive) || (_isManaged != this.isManaged) || (_value != this.currentValue) || _force) {
            ...
            this.textValueContent = (value + '').padStart(5, "0");
            this.setElementVisibility(this.illuminator, this.isManaged);
        }
    }
}
...
class A320_Neo_FCU_VerticalSpeed extends A320_Neo_FCU_Component {
    ...
    set currentState(v) {
        ...
        SimVar.SetSimVarValue("L:A320_NEO_FCU_STATE", "number", this.currentState);
    }
    init() {
        this.textVS = this.getTextElement("VS");
        this.textFPA = this.getTextElement("FPA");
    ...
    onPush() {
        ...
        this.gps.removeCall(this._resetSelectionTimeout);
        this.forceUpdate = true;
    }
    onRotate() {
        if (this.currentState === A320_Neo_FCU_VSpeed_State.Idle || this.currentState === A320_Neo_FCU_VSpeed_State.Selecting) {
            ...
            this.gps.removeCall(this._resetSelectionTimeout);
            this.forceUpdate = true;
            this._resetSelectionTimeout = this.gps.requestCall(() => {
                this.currentState = A320_Neo_FCU_VSpeed_State.Idle;
                this.forceUpdate = true;
            }, 10000);
        }
        else if (this.currentState === A320_Neo_FCU_VSpeed_State.Flying) {
            this.onPull();
        }
    }
    onPull() {
        ...
        if (Simplane.getAutoPilotTRKFPAModeActive()) {
            Coherent.call("AP_VS_VAR_SET_ENGLISH", 2, Simplane.getAutoPilotDisplayedVerticalSpeedHoldValue());
        }
        else {
            Coherent.call("AP_VS_VAR_SET_ENGLISH", 1, Simplane.getAutoPilotDisplayedVerticalSpeedHoldValue());
        }
        SimVar.SetSimVarValue("K:ALTITUDE_SLOT_INDEX_SET", "number", 1);
        SimVar.SetSimVarValue("K:VS_SLOT_INDEX_SET", "number", 1);
        let targetAltitude = Simplane.getAutoPilotDisplayedAltitudeLockValue();
        ...
        if (Math.abs(deltaAltitude) < 100) {
            ...
            Coherent.call("AP_ALT_VAR_SET_ENGLISH", 1, targetAltitude, true);
        }
        SimVar.SetSimVarValue("K:AP_PANEL_VS_ON", "Number", 1);
        this.gps.removeCall(this._resetSelectionTimeout);
        this.forceUpdate = true;
    ...
    update(_deltaTime) {
        ...
        if (this.currentState === A320_Neo_FCU_VSpeed_State.Flying) {
            if (Simplane.getAutoPilotTRKFPAModeActive()) {
                Coherent.call("AP_VS_VAR_SET_ENGLISH", 1, Simplane.convertFPAToVS(SimVar.GetSimVarValue("AUTOPILOT VERTICAL HOLD VAR:2", "feet per minute") / 600));
            }
        }
        ...
        if (Simplane.getAutoPilotTRKFPAModeActive()) {
            this.refresh(true, true, Simplane.getAutoPilotDisplayedVerticalSpeedHoldValue() / 600, this.forceUpdate);
        }
        else {
            this.refresh(true, false, Simplane.getAutoPilotDisplayedVerticalSpeedHoldValue(), this.forceUpdate);
        }
        ...
        if ((_isActive != this.isActive) || (_isFPAMode != this.isFPAMode) || (_value != this.currentValue) || _force) {
            ...
            if (this.isActive && this.currentState != A320_Neo_FCU_VSpeed_State.Idle) {
                ...
                if (this.isFPAMode) {
                    ...
                    this.textValueContent = String.fromCharCode(160) + sign + (fastToFixed((value * 10), 0) + "0").padStart(3, "0");
                }
                else {
                    if (this.currentState === A320_Neo_FCU_VSpeed_State.Zeroing) {
                        ...
                        this.textValueContent = sign + ((Math.floor(value * 0.01) + '').padStart(2, "0")) + "oo";
                    }
                }
    ...
    refresh(_mode, _isHGUnit, _value, _force = false) {
        if ((_mode != this.currentMode) || (_isHGUnit != this.isHGUnit) || (_value != this.currentValue) || _force) {
            ...
            if (this.currentMode == "STD") {
                diffAndSetStyle(this.standardElem, StyleProperty.display, "block");
                diffAndSetStyle(this.selectedElem, StyleProperty.display, "none");
                SimVar.SetSimVarValue("KOHLSMAN SETTING STD", "Bool", 1);
            }
            else {
                diffAndSetStyle(this.standardElem, StyleProperty.display, "none");
                diffAndSetStyle(this.selectedElem, StyleProperty.display, "block");
                ...
                this.textValueContent = (value + '').padStart(4, "0");
            }
        }
    }
}
