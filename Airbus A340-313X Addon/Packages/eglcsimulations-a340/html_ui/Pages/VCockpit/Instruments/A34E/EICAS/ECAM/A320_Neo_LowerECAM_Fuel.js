...
(function (A320_Neo_LowerECAM_Fuel) {
    class JetFuelToggleElement {
        ...
        setState(_isOn, _isActive, _force = false) {
            if ((_isOn != this.isOn) || (_isActive != this.isActive) || _force) {
                ...
                if (this.On != null) {
                    diffAndSetAttribute(this.On, "visibility", this.isOn ? "visible" : "hidden");
                    if (this.isOn) {
                        diffAndSetAttribute(this.On, "class", this.isActive ? "ValvePumpActive" : "ValvePumpInactive");
                    }
                }
                if (this.Off != null) {
                    diffAndSetAttribute(this.Off, "visibility", this.isOn ? "hidden" : "visible");
                    if (!this.isOn) {
                        diffAndSetAttribute(this.Off, "class", !this.isActive ? "ValvePumpActive" : "ValvePumpInactive");
                    }
                }
            }
        }
    ...
    class Page extends Airliners.EICASTemplateElement {
        ...
        update(_deltaTime) {
            ...
            if (isInMetric) {
                diffAndSetText(this.FOBUnit, "KG");
                diffAndSetText(this.fuelFlowUnit, "KG/MIN");
                diffAndSetText(this.middleFuelUnit, "KG");
            }
            else {
                diffAndSetText(this.FOBUnit, "LBS");
                diffAndSetText(this.fuelFlowUnit, "LBS/MIN");
                diffAndSetText(this.middleFuelUnit, "LBS");
            }
        ...
        updateFuelFlow(_unitFactor) {
            ...
            diffAndSetText(this.fuelFlowValue, fastToFixed(totalFuelFlow, 0));
        }
        updateFuelConsumption(_unitFactor) {
            if (this.fuelLevels) {
                ...
                diffAndSetText(this.leftValveValue, fastToFixed(leftConsumption, 0));
                diffAndSetText(this.rightValveValue, fastToFixed(rightConsumption, 0));
                diffAndSetText(this.middleFuelValue, fastToFixed(totalConsumption, 0));
            }
            else {
                this.fuelLevels = SimVar.GetGameVarValue("AIRCRAFT INITIAL FUEL LEVELS", "FuelLevels");
            }
        ...
        updateQuantity(_elem, _simvar, _unitFactor) {
            ...
            diffAndSetText(_elem, fastToFixed(quantity, 0));
        }
        setAPUState(_isOn, _isActive, _force = false) {
            if ((_isOn != this.apuOn) || (_isActive != this.apuActive) || _force) {
                ...
                if (this.apuElement != null) {
                    {
                        diffAndSetAttribute(this.apuElement, "class", this.apuOn ? "active" : "inactive");
                    }
                }
            }
        }
